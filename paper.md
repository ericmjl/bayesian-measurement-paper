...
High throughput (HT) screening experiments are necessary for systematically interrogating biology. Statistical tools have been developed for HT data analysis, for example, the Z- and Z'-factors [@Zhang:1999fr; @Edwards:2015ig; @Lee:2010if; @Sui:2007dc] for assay evaluation and identifying true hits, per-plate Z-score normalization [@Malo:2006kg], ANOVA [@Su:2010gb] and the "three standard deviation" (3-SD) rule-of-thumb [@Zhang:1999fr]. These statistical tools would be useful, if not for the widespread use of small numbers of replicate experiments [@Edwards:2015ig; @Wang:2015ba; @Su:2010gb] risking the measurement and assessment of unnecessarily large numbers of false positives and negatives.
...
## Bayesian credible intervals should replace the commonplace use of Std. Error of Means.
...
A common practice in the life sciences is to report mean ± standard error of the mean (SEM). Part of this is borne out of the convenience in computing these values using statistical software (e.g. GraphPad Prism), and part of may be motivated by the desire to display error bars that show minimal overlap with controls. As shown in Figure @fig:range-estimates, the SEM grossly under-represents the variation in the data. On the other hand, Bayesian and frequentist 95% confidence intervals much more readily capture the data variation.

![Bayesian, frequentist 95% confidence intervals, and error bar widths based on SEM.](./figures/simulation-range-estimates.pdf){#fig:range-estimates}
